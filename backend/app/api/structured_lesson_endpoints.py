...
def get_language_level_with_topics(
    ...
    try:
        print(f"DEBUG: Requesting language='{language}', level='{level}'")  # Debug log
        result = StructuredLessonService.get_language_level_with_topics(db, language, level)
        if not result:
            print(f"DEBUG: No result found for language='{language}', level='{level}'")  # Debug log
            raise HTTPException(status_code=404, detail="Language level not found")
        print(f"DEBUG: Found result for language='{language}', level='{level}'")  # Debug log
        return result
